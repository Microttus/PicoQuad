...
class acsHandle:    
    ...
    def MPU_init(self):
        bus.writeto_mem(address, 25, bytes([7]))
        bus.writeto_mem(address, 107, bytes([1]))
        bus.writeto_mem(address, 26, bytes([0]))
        bus.writeto_mem(address, 27, bytes([24]))
        bus.writeto_mem(address, 56, bytes([1]))
...
class MotorHandle:
    ...
    def ardu_map(self, val, in_min, in_max):
        return int(max(min(self.max_pwm,(val - in_min) * (self.max_pwm - self.min_pwm) // (in_max - in_min) + self.min_pwm), self.min_pwm))
